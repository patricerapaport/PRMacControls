...
open class crowsTable {
    //var controller: NSResponder!
    var table: cmyTable!
    public var rows: [csourceTable]!
    
    public init (table: cmyTable, response: [String]) {
        self.table = table
        if response.count < 3 {
            return
        }
        ...
        for index in 3...response.count-1 {
            ...
            if (params.count > 1) {
                ...
                k = k+1
                if k % nbzones! == 0 {
                    rows.append(csourceTable(parent: self,  response: dict))
                    dict.removeAll()
                    k = 0
                }
            }
    ...
    public init(table: cmyTable, dict: NSDictionary) {
        self.table = table
        
        rows = []
        
        for row in dict.object(forKey: "rows") as! [NSDictionary] {
            rows.append(csourceTable(parent: self, response: row))
        }
    }
    
    public init (table: cmyTable) {
        self.table = table
    }
    ...
    func addRow (at: Int) {
        ...
        rows.insert(csourceTable(parent: self, response:[:]), at: at)
    }
...
open class csourceTable {
    var parent: crowsTable!
    var donnees: [String: String]!
    var ctrls: clisteControles!
    
    public init (parent: crowsTable, response: [String:String]) {
        self.parent = parent
        donnees = response
    }
    
    public init (parent: crowsTable, response: NSDictionary) {
        self.parent = parent
        donnees = [:]
        for (key, value) in response {
            donnees[(key as? String)!] = value is NSNull ? "" : value as? String
        }
    }
    ...
    func valeur (identifier: String, interpreted: Bool!) -> Any {
        ...
        if parent.table.parent.controller.responds (to: methode) {
            //var res =  myController.perform(methode)
            let res = parent.table.parent.controller.perform(methode, with: donnees)
