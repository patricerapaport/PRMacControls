...
class toolbarConfig {
    ...
    init (config: String, table: cmyTable, type: cmyTypesBoutons, visible: Bool, action: String?, etat: Int, selected: Int) {
        self.table = table
        nomConfig = config
        typebouton = type
        isVisible = visible
...
open class cmyToolbarItem: NSToolbarItem {
    ...
    func addConfig (config: String, table: cmyTable, type: cmyTypesBoutons, visible: Bool, action: String?, etat: Int, selected: Int) {
        if tbConfigs == nil {
            tbConfigs = toolbarConfigs()
        }
        tbConfigs.configs.append(toolbarConfig(config: config, table: table, type: type, visible: visible, action: action, etat: etat, selected: selected))
    }
    ...
    func addConfig(config: String, table: cmyTable, type: cmyTypesBoutons) {
        addConfig(config: config, table: table, type: type, visible: false, action: nil, etat: -1, selected: -1)
    }
    ...
    func setConfig (nom: String) {
        if tbConfigs != nil && tbConfigs.configs != nil && tbConfigs.configs.count > 0 {
            for i in 0...tbConfigs.configs.count-1 {
                if tbConfigs.configs[i].nomConfig != nil && tbConfigs.configs[i].nomConfig == nom {
