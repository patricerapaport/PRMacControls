...
open class clisteControles {
        ...
        while index != -1 && index < controles.count-1  {
            if controles[1+index].isLabel || !controles[1+index].isEnabled {
                ...
                if controles[1+index].ctrl is cmyTable {
                    ...
                    if tbl.state != .nonedition && tbl.sourceRow(tbl.rowselected).ctrls != nil {
    ...
    public func verifControl (numcontrol: Int, completion: @escaping(Bool) -> Void) {
    ...
    public func verifControl (tabviewItem: NSTabViewItem) -> Bool {
        for control in controles {
            if control.tabviewItem != nil {
                if !control.verifControl() {
                    return false
